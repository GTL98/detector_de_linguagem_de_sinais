...
Começamos vendo se a porcentagem do valor obtido pelos pontos-chave é maior que o limite, se for, veremos se há alguma coisa na lista de sentenças; e a sentença atual for diferente da última gravada, então é adicionada essa última sentença à lista. O **else** serve para colocar uma sentença aleatória para não começar com a lista vazia, se não é impossível ter comparação sem algo. A parte seguinte dessa lógica serve para mostrar na tela qual é a ação que a IA captou.
