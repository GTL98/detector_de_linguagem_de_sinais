...
Vamos agora compilar o modelo e treiná-lo. Chamamos o método **fit()** e passamos os dados de *X_treino* e *y_treino*; definimos quantidade de épocas em 2000, ,as podemos parar antes se a acurácia começar a cair e chamamos o *tb_callback* para que possamos acompanhar graficamente em tempo real o treino da rede neural. Para conseguirmos ver os gráficos, vamos na tela de onde está a pasta criada pelo *tb_callback*, pasta *train*, e dentro da pasta *treain*: SHIFT + botão direito na tela e selecionamos a opção "Abrir janela de comando aqui". Após aberto do cmd, digite ***tensorboard --logdir=.***. Feito isso, gerará um link para ser copiado e colado no navegador para podermos ver em tempo real o treino da IA. Caso o comando acima no cmd não funcione, didigte isto: ***python -m tensorboard.main --logdir=.***; demorará um pouco para receber o link e o computador ficará bem lento enquanto o treino ocorre. O valor da acurácia dependerá do nivel de processamento do seu comptador. Caso ache que ainda esteja baixo esse valor, aumente a quantidade de épocas a serem percorridas. Depois de uma longa espera, a Etapa 7 está concluída.
...
O primeiro passo a se fazer nessa última etapa é trazer o primeiro loop com a câmera que fizemos (o mesmo da Etapa 2), mas com algumas atualizações para que possamos fazer o reconhecimento dos sinais em tempo real. A primeira variável pegará os 30 frames e colocará em sequência para que seja possível realizar as previsões. A segunda variável concatenará as detecções feitas para fazer as previsões. A terceira variável serve para armazenar as previsões captadas pela IA. E a última variável é um limite para que seja informado a previsão se estiver acima do limite informado, uma margem de confiança. Agora vamos implementar nossa lógica de previsão.

Vamos começar extraindo os pontos-chaves com a função **extrair_pontos_chaves()** (fizemos na Etapa 3) e adicionar esses dados a nossa variável **sequencias** e pegar os últimos 30 frames para fazer a previsão. Como no começo não teremos 30 frames, a previsão será feita somente depois de ter essa quantidade de frames. Completa a lógica de previsão vamos fazer a lógica de visalização.

Começamos vendo se os 10 últimos frames pegos pela IA são iguais; isso diminui a chance da IA reconhecer uma ação falsa durante uma transição de ações. Continuamos vendo se a porcentagem do valor obtido pelos pontos-chave é maior que o limite, se for, veremos se há alguma coisa na lista de sentenças; e a sentença atual for diferente da última gravada, então é adicionada essa última sentença à lista. O **else** serve para colocar uma sentença aleatória para não começar com a lista vazia, se não é impossível ter comparação sem algo. A parte seguinte dessa lógica serve para mostrar na tela qual é a ação que a IA captou. Se quisermos parar por aqui, basta ajeitar o valor do **limite** para que o reconhecimento fique melhor, mas vamos deixar a saída das ações mais bonita. Para isso, vamos criar uma função na célula acima do loop.

Essa função mostrará na tela uma barra por ação indicando a porcentagem da previsão feita pela IA. Essa maneira é boa para vermos qual a porcentagem de leitura de cada ação ensinada à IA.

Depois de 11 etapas, algumas linhas de códigos e muitas horas; a nossa Inteligência Artificial está pronta!!!
